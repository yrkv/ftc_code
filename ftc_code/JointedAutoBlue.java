...
public class JointedAutoBlue extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        navx_device.zeroYaw(); // reset the yaw value to 0
...
//        encoderDriveLaunchAtEnd(0.4, 16, 16, 5, 450);
        ...
        encoderRotateWithGyro(55, 0.3);
        encoderRotateWithGyro(55, 0.3);
        encoderDrive(0.7, 39.5, 39.5, 5);
        encoderRotateWithGyro(20, 1.5);
        encoderRotateWithGyro(20, 1.5);
...
//        encoderDriveSmoothShiftToTape(5, 0.8, 0.15, 25, 13, 10, 10);
        lineColorSensor.enableLed(false);
//        leftMotor.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
//        rightMotor.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        ...
        sleep(100); // wait for the servos to move
...
//        rightLauncherMotor.setPower(0);
    ...
    public void encoderDriveSmoothShiftToTape(int threshold, double speed1, double speed2,
                             double inches1, double inches2, double inches3,
                             double timeoutS) throws InterruptedException {
        int newLeftTarget;
        int newRightTarget;
        ...
        if (opModeIsActive()) {
            ...
            // Determine new target position, and pass to motor controller
            newLeftTarget = leftMotor.getCurrentPosition() + (int)((inches1 + inches2 + inches3) * COUNTS_PER_INCH);
            newRightTarget = rightMotor.getCurrentPosition() + (int)((inches1 + inches2 + inches3) * COUNTS_PER_INCH);
            leftMotor.setTargetPosition(newLeftTarget);
            rightMotor.setTargetPosition(newRightTarget);
            ...
            while (opModeIsActive() &&
                ...
                double speed;
                double dist = (leftMotor.getCurrentPosition() - startLeft + rightMotor.getCurrentPosition() - startRight) / 2.0;

                if (dist < inches1 * COUNTS_PER_INCH) {
                    speed = speed1;
                } else if (dist < inches2 * COUNTS_PER_INCH) {
                    speed = (dist - inches1 * COUNTS_PER_INCH) / (inches2 * COUNTS_PER_INCH) * (speed2 - speed1) + speed1;
                } else if (dist < inches3 * COUNTS_PER_INCH) {
                    speed = speed2;
                } else {
                    speed = 0;
                }
    ...
    public void encoderDrive(double speed1, double speed2,
        ...
        if (opModeIsActive()) {
            ...
            while (opModeIsActive() &&
                ...
                double speed = averagePos / (newLeftTarget - startLeft) * (speed2 - speed1) + speed1;
